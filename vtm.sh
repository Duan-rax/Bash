...
pick_services() {
  ...
  local -a arr=() sel=()
  local n i e
  for n in $found; do arr+=("$n"); done
  # 预选：已有配置按配置来；首次配置则默认全选（检测到的通常就是想管的）
  for i in "${!arr[@]}"; do
    if [[ -n "$SERVICES" ]]; then
      [[ " $SERVICES " == *" ${arr[$i]} "* ]] && sel[$i]=1 || sel[$i]=0
    else sel[$i]=1; fi
  done
  # 配置里有、但这台机器没检测到的，也列出来
  for n in $SERVICES; do
    [[ " ${arr[*]:-} " == *" $n "* ]] || { arr+=("$n"); sel+=(1); }
  done
  if (( ${#arr[@]} == 0 )); then
    echo "  ${CY}未识别到常见服务${C0}" >&2
    ask "手动输入服务名（空格分隔，可留空）" ""
    return
  fi
  while :; do
    echo >&2
    for i in "${!arr[@]}"; do
      if [[ ${sel[$i]} == 1 ]]; then
        printf "    ${CB}%2d)${C0} ${CG}[x]${C0} %s\n" "$((i+1))" "${arr[$i]}" >&2
      else
        printf "    ${CB}%2d)${C0} [ ] %s\n" "$((i+1))" "${arr[$i]}" >&2
      fi
    done
    echo "  ${CD}序号=切换勾选(可空格分隔多个) · a=全选 · z=全不选 · m=手动添加${C0}" >&2
    echo "  ${CB}直接按回车 = 确认并继续${C0}" >&2
    local ans; read -rp "  > " ans
    [[ -z "$ans" ]] && break
    case "$ans" in
      a|A) for i in "${!arr[@]}"; do sel[$i]=1; done
           echo "  ${CG}→${C0} 已全选" >&2; continue ;;
      z|Z) for i in "${!arr[@]}"; do sel[$i]=0; done
           echo "  ${CG}→${C0} 已全部取消" >&2; continue ;;
      m|M) local extra; read -rp "  服务名(空格分隔): " extra
           for e in $extra; do arr+=("$e"); sel+=(1); done; continue ;;
    esac
    local x changed=""
    for x in $ans; do
      [[ "$x" =~ ^[0-9]+$ ]] || continue
      (( x>=1 && x<=${#arr[@]} )) || continue
      sel[$((x-1))]=$(( 1 - sel[$((x-1))] ))
      if (( sel[$((x-1))] )); then changed+="${CG}+${arr[$((x-1))]}${C0} "
      else changed+="${CD}-${arr[$((x-1))]}${C0} "; fi
    done
    if [[ -n "$changed" ]]; then echo "  → $changed" >&2
    else echo "  ${CR}输入无效${C0}" >&2; fi
  done
  local out=""
